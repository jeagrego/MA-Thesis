...
DEFAULT_compas_PREPROCESSING_LABELS = {
    "uniform": "Uniform",
    "reweigh_group_label": "Reweighting",
}


DEFAULT_compas_FAMILY_LABELS = {
    "linucb": "LinUCB",
...
def compas_friendly_label(value: str, labels: dict[str, str] | None = None) -> str:
    """
    Return a friendly label for an COMPAS policy or preprocessing value.
    """
    labels = labels or {}
    return labels.get(str(value), str(value))


def compas_curve_style(
    *,
    policy: str,
    preprocessing: str = "uniform",
) -> dict[str, str]:
    """
    Return a style dictionary for an COMPAS policy and preprocessing combination.
    """
...
def aggregate_compas_curve(
    ...
    *,
    ...
    """
    Aggregate an COMPAS temporal curve across seeds.
...
def draw_compas_curve(
    ...
    *,
    ...
    """
    Draw an COMPAS temporal mean curve with pointwise 95% CI bands.
    """
    if curve.empty:
        return
...
def plot_compas_preprocessing_fairness(
    ...
    *,
    ...
    """
    Plot fairness metrics for COMPAS policies with different preprocessing methods.
    """
    ...
    fig.suptitle(
        f"COMPAS | sensitive = race binary | {family_label} | Preprocessing comparison",
        fontweight="bold",
        y=1.02,
    )
    ...
    for ax, metric, ylabel in [
        ...
        summary = aggregate_compas_curve(df, metric, ["policy", "preprocessing"])
        ...
        for policy in policies:
            for preprocessing in preprocessings:
                ...
                label = (
                    f"{compas_friendly_label(policy, policy_labels)} | "
                    f"{compas_friendly_label(preprocessing, preprocessing_labels)}"
                )

                style = compas_curve_style(policy=policy, preprocessing=preprocessing)
                draw_compas_curve(ax, curve, label=label, **style)
    ...
    output_path = fig_dir / f"compas__race_binary_{family}_preprocessing_fairness.png"
...
def plot_compas_inprocessing_fairness(
    temporal_df: pd.DataFrame,
    *,
    family: str,
    family_label: str,
    ...
    """
    Plot fairness metrics for COMPAS policies with different in-processing methods.
    """
    df = temporal_df[temporal_df["family"] == family].copy() if "family" in temporal_df.columns else temporal_df.copy()
    ...
    fig.suptitle(
        f"COMPAS | sensitive = race binary | {family_label} | In-processing comparison",
        fontweight="bold",
        y=1.02,
    )
    ...
    for ax, metric, ylabel in [
        ...
        summary = aggregate_compas_curve(df, metric, ["policy"])

        for policy in policies:
            curve = summary[summary["policy"] == policy].sort_values("t")
            label = compas_friendly_label(policy, policy_labels)
            style = compas_curve_style(policy=policy, preprocessing="uniform")
            draw_compas_curve(ax, curve, label=label, **style)
    ...
    fig_dir = Path(fig_dir)
    fig_dir.mkdir(parents=True, exist_ok=True)
    output_path = fig_dir / f"compas__race_binary_{family}_inprocessing_fairness.png"
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    print("Saved:", output_path)
    if show:
        plt.show()
...
def plot_compas_performance_metric(
    temporal_df: pd.DataFrame,
    *,
    family: str,
    family_label: str,
    ...
    """
    Plot a performance metric for COMPAS policies with different preprocessing methods.
    """
    df = temporal_df[temporal_df["family"] == family].copy() if "family" in temporal_df.columns else temporal_df.copy()
    summary = aggregate_compas_curve(df, metric_col, ["policy", "preprocessing"])
    ...
    ax.set_title(
        f"COMPAS | sensitive = race binary | {family_label} | {ylabel} over time",
        fontweight="bold",
    )
    ax.set_xlabel("Rounds")
    ax.set_ylabel(ylabel)
    ...
    fig_dir = Path(fig_dir)
    fig_dir.mkdir(parents=True, exist_ok=True)
    output_path = fig_dir / f"compas__race_binary_{family}_{filename_suffix}.png"
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    print("Saved:", output_path)
    if show:
        plt.show()
...
def plot_compas_linucb_inprocessing_average_reward(
    temporal_df: pd.DataFrame,
    *,
    fig_dir: str | Path,
    policy_labels: dict[str, str] | None = None,
    ...
    """
    Plot the COMPAS LinUCB in-processing comparison for average reward.
    ...
    if df.empty:
        raise ValueError(
            "No COMPAS LinUCB/FairLinUCB uniform rows found in temporal_df."
        )

    summary = aggregate_compas_curve(
        df,
        "average_reward",
        ["policy"],
    )
    ...
    fig.suptitle(
        "COMPAS | sensitive = race binary | LinUCB | In-processing average reward",
        fontweight="bold",
        y=1.02,
    )
    ...
    for policy in policies:
        ...
        label = compas_friendly_label(
            policy,
            policy_labels,
        )

        style = compas_curve_style(
            policy=policy,
    ...
    output_path = (
        fig_dir
        / "compas_race_binary_linucb_inprocessing_average_reward_uniform.png"
    )
...
def plot_compas_linucb_postprocessing_average_reward(
    ...
    *,
    ...
    """
    Plot the COMPAS LinUCB post-processing comparison for average reward.
    ...
    if df.empty:
        raise ValueError(
            "No COMPAS FairLinUCB/FairLinUCB+PP uniform rows found "
    ...
    fig.suptitle(
        "COMPAS | sensitive = race binary | LinUCB | Post-processing average reward",
        fontweight="bold",
        y=1.02,
    )
    ...
    output_path = (
        fig_dir
        / "compas_race_binary_linucb_postprocessing_average_reward_uniform.png"
    )
...
def plot_compas_family_figure_set(
    *,
    temporal_df: pd.DataFrame,
    family: str,
    family_label: str,
    ...
    """
    Generate the five standard online COMPAS figures for one policy family.
    """
    paths = []

    paths.append(
        plot_compas_preprocessing_fairness(
            temporal_df,
            family=family,
    ...
    paths.append(
        plot_compas_inprocessing_fairness(
            temporal_df,
            family=family,
            family_label=family_label,
            policies=policies,
    ...
    paths.append(
        plot_compas_performance_metric(
            temporal_df,
            family=family,
            family_label=family_label,
...
def plot_compas_postprocessing_metric(
    ...
    *,
    ...
    """
    Plot fairness metrics for COMPAS policies with different post-processing methods.
    """
    ...
    ax.set_title(
        f"COMPAS | sensitive = race binary | {family_label} | Post-processing {ylabel}",
        fontweight="bold",
    )
    ...
    fig_dir = Path(fig_dir)
    fig_dir.mkdir(parents=True, exist_ok=True)
    output_path = fig_dir / f"compas__race_binary_{family}_postprocessing_{filename_suffix}.png"
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    print("Saved:", output_path)
    if show:
        plt.show()
...
def plot_compas_postprocessing_summary(
    *,
    postproc_df: pd.DataFrame,
    family: str,
    family_label: str,
    ...
    """
    return [
        plot_compas_postprocessing_metric(
            postproc_df,
            family=family,
            family_label=family_label,
            metric="DP_gap",
...
compas_FINAL_METRIC_DISPLAY_NAMES = {
...
def _ensure_compas_plot_metric_columns(dataframe: pd.DataFrame) -> pd.DataFrame:
    """
...
def _compas_postprocessed_policy_name(fair_policy: str) -> str:
    """
...
def prepare_compas_final_plot_dataframe(
    *,
    ...
    """
    policy_labels = DEFAULT_compas_POLICY_LABELS | (policy_labels or {})
    preprocessing_labels = DEFAULT_compas_PREPROCESSING_LABELS | (
        preprocessing_labels or {}
    )

    endpoint = _ensure_compas_plot_metric_columns(endpoint_df)
    postproc = _ensure_compas_plot_metric_columns(postproc_df)
    ...
    postprocessed_policy = _compas_postprocessed_policy_name(fair_policy)
...
def aggregate_compas_final_metric(
    dataframe: pd.DataFrame,
    *,
    metric: str,
) -> pd.DataFrame:
...
def plot_compas_final_metric_dot_panel(
    *,
    endpoint_df: pd.DataFrame,
    postproc_df: pd.DataFrame,
    family: str,
    ...
    """
    combined, method_order = prepare_compas_final_plot_dataframe(
    ...
    fig.suptitle(
        f"COMPAS | sensitive = race binary | {family_label} | {title_suffix}",
        fontweight="bold",
        y=1.02,
    )

    for ax, metric in zip(axes, metrics):
        summary = aggregate_compas_final_metric(
            combined,
            metric=metric,
        )
        ...
        display_metric = compas_FINAL_METRIC_DISPLAY_NAMES.get(
            metric,
            metric,
        )
        ...
        if metric in compas_LOWER_IS_BETTER:
            ax.set_xlabel(
                "Mean ± 95% CI\n(lower is better)"
            )
            ax.invert_xaxis()
    ...
    output_path = (
        fig_dir
        / f"compas__race_binary_{family}_{filename_suffix}.png"
    )
...
def plot_compas_final_dot_plots(
    *,
    endpoint_df: pd.DataFrame,
    postproc_df: pd.DataFrame,
    family: str,
    ...
    """
    Generate final predictive and fairness dot plots for one COMPAS policy family.
    """
    return [
        plot_compas_final_metric_dot_panel(
            endpoint_df=endpoint_df,
            postproc_df=postproc_df,
            family=family,
            family_label=family_label,
...
def _add_compas_online_baseline_to_postproc_horizon(
    *,
    postproc_horizon_df: pd.DataFrame,
    temporal_df: pd.DataFrame,
    family: str,
...
def plot_compas_postprocessing_over_horizon(
    postproc_horizon_df: pd.DataFrame,
    *,
    temporal_df: pd.DataFrame,
    family: str,
    ...
    df = _add_compas_online_baseline_to_postproc_horizon(
    ...
    df = _ensure_compas_plot_metric_columns(df)
    ...
    fig.suptitle(
        f"COMPAS | sensitive = race binary | {family_label} | Post-processing comparison",
        fontweight="bold",
        y=1.02,
    )
    ...
    output_path = (
        fig_dir
        / f"compas__race_binary_{family}_postprocessing_over_horizon.png"
    )
...
def plot_compas_postprocessing_over_horizon(
    *,
    ...
    """
    Plot COMPAS held-out post-processing metrics over training horizons.

    The figure shows six curves:
    - baseline policy with uniform preprocessing;
    - baseline policy with reweighting;
    - fairness-aware policy with uniform preprocessing;
    - fairness-aware policy with reweighting;
    - post-processed fairness-aware policy with uniform preprocessing;
    - post-processed fairness-aware policy with reweighting.

    The baseline and fairness-aware online curves are reconstructed from
    temporal_df at the closest available checkpoint <= each requested horizon.
    The post-processed curves are taken from postproc_horizon_df.
    """
    if temporal_df.empty:
        raise ValueError("temporal_df is empty.")
    ...
    def _label_policy(policy: str) -> str:
        return policy_labels.get(str(policy), str(policy))

    def _label_preprocessing(preprocessing: str) -> str:
        return preprocessing_labels.get(str(preprocessing), str(preprocessing))

    def _style(policy: str, preprocessing: str) -> dict[str, str]:
        policy_text = str(policy)

        if policy_text == baseline_policy:
            color = "blue"
        elif policy_text == fair_policy:
            color = "red"
        elif policy_text == f"{fair_policy}+PP":
            color = "green"
        else:
            color = "black"
    ...
    def _ensure_metric_aliases(dataframe: pd.DataFrame) -> pd.DataFrame:
        out = dataframe.copy()

        if "average_reward" not in out.columns and "avg_reward" in out.columns:
            out["average_reward"] = out["avg_reward"]

        if "cumulative_prediction_error" not in out.columns and "cumulative_regret" in out.columns:
            out["cumulative_prediction_error"] = out["cumulative_regret"]

        return out

    temporal = _ensure_metric_aliases(temporal_df)
    postproc = _ensure_metric_aliases(postproc_horizon_df)

    if "horizon" not in postproc.columns:
        if "t" in postproc.columns:
            postproc = postproc.copy()
            postproc["horizon"] = postproc["t"]
        else:
            raise KeyError("postproc_horizon_df must contain either 'horizon' or 't'.")

    temporal = temporal[
        (temporal["family"] == family)
        & (temporal["policy"].isin([baseline_policy, fair_policy]))
        & (temporal["preprocessing"].isin(preprocessings))
    ].copy()
    ...
    if temporal.empty:
        raise ValueError(f"No temporal rows available for family={family}.")

    if postproc.empty:
        raise ValueError(f"No post-processing horizon rows available for family={family}.")

    online_rows: list[dict] = []

    for horizon in horizons:
        available = temporal[temporal["t"] <= int(horizon)].copy()
    ...
    online_horizon_df = pd.DataFrame(online_rows)
    ...
    if combined.empty:
        raise ValueError(f"No combined post-processing horizon data for family={family}.")
    ...
    for ax, metric, ylabel in [
        ...
        if metric not in combined.columns:
            raise KeyError(
                f"Metric '{metric}' not found. "
                f"Available columns are: {combined.columns.tolist()}"
            )
        ...
        for policy in plot_policies:
            for preprocessing in preprocessings:
                ...
                if curve.empty:
                    print(
                        "Missing post-processing horizon curve:",
                        family,
                        policy,
                        preprocessing,
                    )
                ...
                label = (
                    f"{_label_policy(policy)} | "
                    f"{_label_preprocessing(preprocessing)}"
                )
    ...
    output_path = (
        fig_dir
        / f"compas_race_binary_{family}_postprocessing_over_horizon.png"
    )
